    ...
    <>
      ...
      <div className="w-full max-w-[1600px] mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
        ...
        <div className="lg:hidden mb-4 sm:mb-6">
          ...
          <div className="flex overflow-x-auto pb-4 gap-3 no-scrollbar snap-x snap-mandatory">
            {QR_TYPES.map(type => (
              <button
                key={type.id}
                onClick={() => setActiveType(type.id)}
                className={`flex-shrink-0 snap-start flex items-center gap-2 px-4 py-3 rounded-2xl border-2 transition-all font-bold whitespace-nowrap ${activeType === type.id
                  ? 'bg-blue-600 border-blue-600 text-white shadow-lg shadow-blue-500/20'
                  : 'bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 text-gray-600 dark:text-gray-300'
                  }`}
              >
                <type.icon className={`w-4 h-4 ${activeType === type.id ? 'text-white' : 'text-gray-400'}`} />
